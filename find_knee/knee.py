class KneeThersholdFinder:
    ...
    def __init__(self, x_values: Union[list, np.ndarray], y_values: Union[list, np.ndarray, None] = None, clean_data: bool = True):
        ...
        if isinstance(x_values, np.ndarray) and y_values is None:
            ...
            assert x_values.shape[1] > 2, "Too few points to find a knee."
            self.data = x_values
        else:
            assert len(x_values) == len(y_values), "x_values and y_values must have the same length."
            assert len(x_values) > 2, "Too few points to find a knee."
    ...
    def find_knee(self) -> np.ndarray:
        ...
        knee_position = np.argmax(magnitudes)
        self.knee = self.data[:, knee_position]
        return self.knee, magnitudes
